...
<x-taste-ui::select.wrapper :$label :$error :$computed :$hint loading>
    <x-slot name="alpine">
        tasteui_selectSearchable(@js($request), @entangle($computed), @js($selectable), @js($multiple), @js($placeholder))
    </x-slot>
    <x-slot name="header">
        <div class="flex gap-2">
            <template x-if="!multiple || quantity === 0">
                <span @class(['truncate font-medium', 'text-red-500' => $error]) x-bind:class="{ 'text-gray-400': empty, 'text-gray-600': !empty }" x-text="placeholder"></span>
            </template>
            <template x-if="multiple && quantity > 0">
