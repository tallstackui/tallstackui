...
| hover     | bool\|null   | false        | Opens on `pointerenter` and closes on `pointerleave` with a 300ms grace period. Restricted to `pointerType === 'mouse'`, so touch keeps the click behavior; the click toggle keeps working alongside                                                                                          |
