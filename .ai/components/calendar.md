...
| format          | string\|null                | `'YYYY-MM-DD'`            | Accepted but never read: the calendar is inline and has no input to render a formatted value, so the model is always written as `YYYY-MM-DD`, or `YYYY-MM` under `month-year-only`.                                                              |
...
| box.picker.expanded       | Minimum height bound on the panel while a picker is open (`min-h-[17rem]`), keeping room under `month-year-only`.                                                                                          |
| box.picker.wrapper.first  | Absolute overlay covering the calendar card while a picker is open, matching the date picker.                                                                                                              |
| box.picker.wrapper.second | Month/year picker flex wrap inside the overlay.                                                                                                                                                            |
| box.picker.wrapper.third  | Month/year picker top bar inside the overlay.                                                                                                                                                              |
...
| box.picker.today          | "Today" shortcut button inside the picker (plain text, inheriting the header typography, matching the date picker).                                                                                        |
...
- **JavaScript bundle** — Calendar's Alpine factory is bundled inside `tallstackui-date.js` (shared with `<x-date>` and `<x-time>`) because they all share the same date helper.
