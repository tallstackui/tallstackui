...
An individual action item within a Dial speed dial component. Displays an icon button with an optional tooltip label, and renders as a link or button depending on whether an href is provided. Each item can have its own color and style, and clicking an item automatically closes the parent dial.
...
<x-dial>
    ...
    <x-dial.items icon="trash" label="Delete" color="red" style="light" />
    <x-dial.items icon="share" label="Share" color="green" style="outline" />
</x-dial>
```
...
| color         | string\|null | null    | Individual color theme for the item; falls back to a neutral style when not set |
| style         | string\|null | 'solid' | Color style variant applied with `color` (solid, light, outline)                |
...
| item.base          | Action button layout styles (dimensions, shadow, focus, cursor, transition)         |
| item.default       | Neutral background/text styles applied when no `color` is set                       |
| item.bordered      | Border width applied when a `color` is set (used by the outline style)              |
