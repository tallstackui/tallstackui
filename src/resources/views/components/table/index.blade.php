            ...
            <table class="{{ $personalize['table.base'] }}" @if ($livewire && $loading) wire:loading.class="{{ $personalize['loading.table'] }}" @endif>
                ...
                @if (!$headerless)
                    <thead @class(['uppercase', $personalize['table.thead.normal'] => !$striped, $personalize['table.thead.striped'] => $striped])>
                        <tr>
                            @if ($expandable)
                                <th @class(['w-8', $personalize['table.th']])></th>
                            @endif
                            @if ($selectable)
                ...
                @else
                    @forelse ($rows as $key => $value)
                                ...
                                <td class="{{ $personalize['table.td'] }}">
                                    @isset($sub_table)
                                        <button 
                                            ...
                                            class="{{ $personalize['subTable.expandButton'] }} cursor-pointer"
                                        >
                                            <x-dynamic-component 
                        ...
                        {{-- Expandable Sub-Table Row (Slot-based) --}}
                        @if ($expandable)
                            @isset($sub_table)
                                <tr x-show="isRowExpanded('{{ $id }}')" x-cloak @if ($livewire) wire:key="sub-{{ $id }}" @endif class="{{ $personalize['subTable.wrapper'] }}">
