...
<div
    x-data="tallstackui_table({!! $entangle !!}, @js($ids()), @js($selectable))"
    ...
    <div class="{{ $personalize["wrapper"] }}">
        <div class="{{ $personalize["table.wrapper"] }}">
            ...
            >
                ...
                @if (! $headerless)
                    <thead
                        @class(["uppercase", $personalize["table.thead.normal"] => ! $striped, $personalize["table.thead.striped"] => $striped])
                    >
                        <tr>
                            @if ($selectable)
                                <th
                                    @class(["w-6", $personalize["table.th"]])
                                    wire:key="checkall-{{ implode(",", $ids()) }}"
                                >
                                    <x-dynamic-component
                                        :component="TallStackUi::prefix('checkbox')"
                                        x-ref="checkbox"
                ...
                <tbody class="{{ $personalize["table.tbody"] }}">
                    ...
                    @else
                        @forelse ($rows as $key => $value)
                            ...
                            <tr
                                @class([$personalize["table.tr"], "dark:bg-dark-800/50 bg-gray-50" => $striped && $loop->index % 2 === 0])
                            ...
                            >
                                ...
                                @foreach ($headers as $header)
                                    ...
                                    @isset(${"column_" . $row})
                                        <td
                                            @if ($clickable) x-on:click.prevent="redirect(@js($url), @js($blank))" @endif
                                            @class([$personalize["table.td"], "cursor-pointer" => $clickable])
                                        >
                                            {{ ${"column_" . $row}($value) }}
                                        </td>
                                    @else
