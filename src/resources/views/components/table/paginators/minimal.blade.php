...
$slot = 'relative inline-flex min-w-8 items-center justify-center px-2 py-1.5 text-sm leading-5 outline-hidden transition duration-200 focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:rounded-sm';
$idle = 'cursor-pointer font-medium text-gray-400 hover:text-gray-900 dark:text-dark-400 dark:hover:text-dark-100';
$current = 'cursor-default select-none font-semibold text-primary-600 after:absolute after:inset-x-2 after:-bottom-1 after:h-0.5 after:rounded-full after:bg-primary-600 dark:text-primary-400 dark:after:bg-primary-400';
$separator = 'cursor-default select-none font-medium text-gray-300 dark:text-dark-500';

$nav = 'inline-flex size-8 shrink-0 items-center justify-center rounded-sm outline-hidden transition duration-200 focus-visible:ring-2 focus-visible:ring-primary-500';
$navIdle = 'cursor-pointer text-gray-400 hover:text-gray-900 dark:text-dark-400 dark:hover:text-dark-100';
$navOff = 'cursor-not-allowed select-none text-gray-200 dark:text-dark-500';

$button = 'relative inline-flex select-none items-center px-1 py-2 text-sm font-medium leading-5 outline-hidden transition duration-200 focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:rounded-sm';
$enabled = 'cursor-pointer text-gray-500 underline-offset-4 hover:text-gray-900 hover:underline dark:text-dark-300 dark:hover:text-dark-100';
$disabled = 'cursor-not-allowed text-gray-300 dark:text-dark-500';
@endphp

@if ($paginator->hasPages())
    <div class="mt-4">
        <nav role="navigation" aria-label="Pagination Navigation">
            <div @class(['flex flex-1 gap-x-6', 'justify-end' => $simple, 'justify-between sm:hidden' => !$simple])>
                <span>
            ...
            <div class="hidden sm:flex sm:items-center sm:justify-between">
                <div class="mr-4">
                    <p class="text-sm leading-5 text-gray-400 dark:text-dark-500">
                        <span>{!! trans('Showing') !!}</span>
                        <span class="font-semibold text-gray-700 dark:text-dark-200">{{ $paginator->firstItem() }}</span>
                        <span>{!! trans('to') !!}</span>
                        <span class="font-semibold text-gray-700 dark:text-dark-200">{{ $paginator->lastItem() }}</span>
                        <span>{!! trans('of') !!}</span>
                        <span class="font-semibold text-gray-700 dark:text-dark-200">{{ $paginator->total() }}</span>
