            ...
            <table class="{{ $customization['table.base'] }}"
                ...
                @if (!$headerless)
                    ...
                    <tr>
                        ...
                        @endif
                        @if ($selectable)
                            <th @class([$customization['table.th-actions-width'], $customization['table.th'.$compacted]]) wire:key="checkall-{{ implode(',', $ids()) }}">
                ...
                @if (is_array($rows) && (count($rows) === 1 && empty($rows[0])))
                    <tr>
                        <td class="{{ $customization['empty'.$compacted] }}" colspan="100%">
                            @if ($empty)
                                {{ $empty }}
                            @else
                    ...
                    @forelse ($rows as $key => $value)
                        ...
                        <tr @class([
                            ...
                            @foreach($headers as $header)
                                ...
                                @endphp
                                @isset(${"column_".$row})
                                    <td @if ($clickable) x-on:click.prevent="redirect(@js($url), @js($blank))" @endif @class([$customization['table.td'.$compacted], $customization['cell-clickable'] => $clickable])>
                                        {{ ${"column_".$row}($value) }}
                                    </td>
                                @else
                                    <td @if ($clickable) x-on:click.prevent="redirect(@js($url), @js($blank))" @endif @class([$customization['table.td'.$compacted], $customization['cell-clickable'] => $clickable])>
                        ...
                        @if ($expandable)
                            @isset($sub_table)
                                <tr x-show="expanded('{{ $id }}')" x-cloak @if ($livewire) wire:key="sub-{{ $id }}"
                                    ...
                                    <td colspan="100%" class="{{ $customization['expandable.content'.$compacted] }}">
                                        {{ $sub_table($value) }}
                                    </td>
                                </tr>
                            @endisset
