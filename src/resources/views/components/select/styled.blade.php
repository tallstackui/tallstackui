...
<div
    ...
    wire:ignore
>
    <div hidden x-ref="options">
        {{ TallStackUi::blade()->json($options) }}
    </div>
    @if ($request["params"] ?? null)
        <div hidden x-ref="params">
    ...
    <div class="relative" x-on:click.outside="show = false">
        ...
        >
            <div class="{{ $personalize["input.content.wrapper.first"] }}">
                <div
                    class="{{ $personalize["input.content.wrapper.second"] }}"
                >
                    ...
                    <div x-show="empty || !multiple">
                        <div
                            class="{{ $personalize["items.placeholder.wrapper"] }}"
                        >
                            <img
                                x-bind:src="image"
                                class="{{ $personalize["items.image"] }}"
                                x-show="image"
                            />
                            <span
                                @class(["text-red-500 dark:text-red-500" => $error])
                                x-bind:class="{
                    ...
                    <div
                        wire:ignore
                        class="{{ $personalize["items.wrapper"] }}"
                        x-show="multiple && quantity > 0"
                    >
                        <template
                            x-for="(select, index) in selects"
                            :key="index"
                        >
                            <a class="cursor-pointer">
                                <div
                                    class="{{ $personalize["items.multiple.item"] }}"
                                >
                                    <div
                                        class="{{ $personalize["items.multiple.label.wrapper"] }}"
                                    >
                                        <template x-if="select.image">
                                            <img
                                                x-bind:src="select.image"
                                                class="{{ $personalize["items.multiple.image"] }}"
                                            />
                                        </template>
                                        <span
                                            class="{{ $personalize["items.multiple.label"] }}"
                                            x-text="select[selectable.label] ?? select"
                                        ></span>
                                    </div>
                                    @if (! $disabled)
                                        <div
        ...
        <x-dynamic-component
            :component="TallStackUi::prefix('floating')"
        ...
        >
            ...
            <ul
                class="{{ $personalize["box.list.wrapper"] }}"
                dusk="tallstackui_select_options"
                role="listbox"
                x-ref="list"
                wire:ignore.self
            >
                @if ($request)
                    <div
                        x-show="loading"
                        class="{{ $personalize["box.list.loading.wrapper"] }}"
                    >
                        <x-tallstack-ui::icon.generic.loading
                            class="{{ $personalize['box.list.loading.class'] }}"
                        />
                    </div>
                @endif
                ...
                @if ($grouped)
                    <template
                        x-for="(option, index) in available"
                        :key="option.__tsui_key ?? index"
                    >
                        <li>
                            <div
                                class="{{ $personalize["box.list.grouped.wrapper"] }}"
                            >
                                <div
                                    class="{{ $personalize["box.list.grouped.options"] }}"
                                >
                                    <div
                                        class="{{ $personalize["box.list.grouped.base"] }}"
                                    >
                                        <img
                                            class="{{ $personalize["box.list.grouped.image"] }}"
                                            x-bind:src="option.image"
                                            x-show="option.image"
                                        />
                                        <div
                                            class="{{ $personalize["box.list.grouped.description.wrapper"] }}"
                                        >
                                            <span
                                                x-text="option[selectable.label] ?? option"
                                            ></span>
                                            <span
                                                class="{{ $personalize["box.list.grouped.description.text"] }}"
                            ...
                            <template
                                x-for="(item, index) in option.value"
                                :key="index"
                            >
                                <li
                                    ...
                                    x-bind:class="{
                                        '{{ $personalize["box.list.item.selected"] }}': selected(item),
                                        '{{ $personalize["box.list.item.disabled"] }}': item.disabled === true,
                                    }"
                                    role="option"
                                    class="{{ $personalize["box.list.item.wrapper"] }}"
                                >
                                    <div
                                        class="{{ $personalize["box.list.item.grouped"] }}"
                                    >
                                        <div
                                            class="{{ $personalize["box.list.item.base"] }}"
                                        >
                                            <img
                                                class="{{ $personalize["box.list.item.image"] }}"
                                                x-bind:src="item[selectable.image]"
                                                x-show="item[selectable.description]"
                                            />
                                            <div
                                                class="{{ $personalize["box.list.item.description.wrapper"] }}"
                                            >
                                                <span
                                                    x-text="item[selectable.label] ?? item"
                                                ></span>
                                                <span
                                                    class="{{ $personalize["box.list.item.description.text"] }}"
                                                    x-show="item[selectable.description]"
                                                    x-text="item[selectable.description]"
                                                ></span>
                                            </div>
                                        </div>
                                        <div
                ...
                @else
                    ...
                    >
                        <li
                            ...
                            x-bind:class="{
                                '{{ $personalize["box.list.item.selected"] }}': selected(option),
                                '{{ $personalize["box.list.item.disabled"] }}': option.disabled === true,
                            }"
                            role="option"
                            class="{{ $personalize["box.list.item.wrapper"] }}"
                        >
                            <div
                                class="{{ $personalize["box.list.item.options"] }}"
                            >
                                <div
                                    class="{{ $personalize["box.list.item.base"] }}"
                                >
                                    ...
                                    >
                                        ...
                                        <span
                                            ...
                                            x-text="option[selectable.description]"
                                        ></span>
                                    </div>
                                </div>
                                <div
                ...
                @endif
                <li
                    x-show="@js($common) === true && available.length >= 10"
