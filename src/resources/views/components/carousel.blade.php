...
<div
    x-data="tallstackui_carousel(@js($images), @js($cover), @js($autoplay), @js($interval), @js($withoutLoop))"
        ...
        >
            <template x-for="(image, index) in images" :key="index">
                <div
                    ...
                    class="{{ $personalize["images.wrapper.first"] }}"
                    x-transition.opacity.duration.1000ms
                >
                    <a
                        x-bind:href="image.url ?? '#'"
                        x-bind:target="image.target"
                    >
                        <template x-if="image.title">
                            <div
                                @class([$personalize["images.wrapper.second"], "rounded-xl" => $round])
                            >
                                <h3
                                    class="{{ $personalize["images.content.title"] }}"
                                    x-text="image.title"
                                ></h3>
                                <p
                                    class="{{ $personalize["images.content.description"] }}"
                                    x-text="image.description"
                                ></p>
                            </div>
                        </template>
                        <img
                            @class([$personalize["images.base"], "rounded-xl" => $round])
        ...
        @if (! $withoutIndicators)
            <div class="{{ $personalize["indicators.wrapper"] }}">
                <template x-for="(image, index) in images">
                    <button
                        class="{{ $personalize["indicators.buttons.base"] }}"
                        x-on:click="(current = index + 1), reset()"
                        x-bind:class="
                            [
                                current === index + 1
                                    ? '{{ $personalize["indicators.buttons.current"] }}'
                                    : '{{ $personalize["indicators.buttons.inactive"] }}',
                            ]
                        "
                    ></button>
                </template>
