...
<div>
    ...
    <div @class(['mt-1', 'flex' => $type === 'input'])>
        @if ($type === 'input')
            <div class="w-full flex" x-data="tallstackui_clipboard(@js($text), @js($hash), @js($type), @js($placeholders['button']))">
                @if ($left)
        ...
        @endif
        @if ($type === 'textarea')
            <div x-data="tallstackui_clipboard(@js($text), @js($hash), @js($type), @js($placeholders['button']))" class="relative min-w-0 flex-1">
        ...
        @endif
        @if ($type === 'icon')
            <div x-data="tallstackui_clipboard(@js($text), @js($hash), @js($type), @js($placeholders['button']))" class="inline-flex">
