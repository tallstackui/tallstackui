...
>
    <nav
        @if ($variation === 'panels') class="overflow-hidden rounded-md" @endif
    >
        <ul role="list" @class($personalize["wrapper." . $variation])>
            <template x-for="item in steps">
                <x-dynamic-component
                    component="tallstack-ui::step.variations.{{ $variation }}"
                    :$personalize
                    :$navigate
                />
            </template>
    ...
    @if ($helpers)
        <div
            class="{{ $personalize["helpers.wrapper"] }}"
            ...
            <div>
                @if ($navigatePrevious)
                    <button
                        ...
                        x-on:click="
                            selected--
                            $refs.buttons.dispatchEvent(
                                new CustomEvent('change', { detail: { step: selected } }),
                            )
                        "
