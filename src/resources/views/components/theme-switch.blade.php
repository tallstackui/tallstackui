...
<div wire:ignore>
    <button type="button"
            ...
            @if (!$simple)
            x-bind:class="{ 'bg-primary-500': darkTheme === true, 'bg-gray-200': darkTheme === false }"
            @endif
            @class([$personalize['switch.button'] => !$simple])>
         <div @class([$personalize['switch.wrapper'] => !$simple, $personalize['simple.wrapper'] => $simple])
              @if (!$simple) 
              x-bind:class="{ 'translate-x-5': darkTheme === true, 'translate-x-0': darkTheme === false }" 
              @endif>
            <span @class($personalize['span'])
               ...
               <x-dynamic-component :component="TallStackUi::component('icon')"
                                    ...
                                    @class([$personalize['switch.icon'] => !$simple, $personalize['simple.icon'] => $simple]) />
            </span>
            <span @class($personalize['span'])
                  aria-hidden="true"
