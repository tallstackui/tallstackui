    ...
    x-data="{
        themeSwitch() {
            this.$el.dispatchEvent(
                new CustomEvent('theme', { detail: { darkTheme: darkTheme } }),
            )
        },
    }"
>
    <button
        ...
        @if (!$onlyIcons) x-bind:class="{
            '{{ $personalize["switch.on"] }}': darkTheme === true,
            '{{ $personalize["switch.off"] }}': darkTheme === false,
        }" @endif
    ...
    >
        <div
            ...
            @if (!$onlyIcons) x-bind:class="{
                '{{ $personalize["switch.translate." . $size] }}': darkTheme === true,
