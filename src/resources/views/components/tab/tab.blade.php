...
<div
    x-data="{ selected: @if (!$selected) {!! TallStackUi::blade($attributes, $livewire)->entangle() !!} @else @js($selected) @endif, tabs: [] }"
...
>
    @if (! $scrollOnMobile)
        <div class="{{ $personalize["base.padding"] }}">
            <select
                x-model="selected"
                class="{{ $personalize["base.select"] }}"
                aria-label="Select a tab"
                x-on:change="
                    $refs.ul.dispatchEvent(
                        new CustomEvent('navigate', { detail: { select: selected } }),
                    )
                "
            >
                <template x-for="item in tabs">
                    <option
                        x-bind:value="item.tab"
                        x-text="item.tab"
    ...
    <ul
        role="tablist"
        @class([$personalize["base.body"], "hidden sm:flex" => ! $scrollOnMobile])
        {{ $attributes->only("x-on:navigate") }}
        x-ref="ul"
    >
        <template x-for="item in tabs">
            <li
                ...
                x-on:click="
                    selected = item.tab
                    $refs.ul.dispatchEvent(
                        new CustomEvent('navigate', { detail: { select: item.tab } }),
                    )
                "
                x-on:keypress.enter="
                    selected = item.tab
    ...
    <hr
        @class([$personalize["base.divider"], "hidden sm:block" => ! $scrollOnMobile])
    />
