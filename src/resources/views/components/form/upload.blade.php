...
>
        ...
        <template x-teleport="body">
            ...
            >
                <div class="{{ $personalize["preview.wrapper"] }}">
                    <button
                        class="{{ $personalize["preview.button.base"] }}"
                        x-on:click="
                            preview = false
                            $nextTick(() => (show = true))
                        "
                    ...
                    <img
                        x-bind:src="image"
                        class="{{ $personalize["preview.image"] }}"
                    />
                </div>
            </div>
        </template>
    ...
    >
        @if (! $static)
            <div
                @class(["flex w-full flex-col items-center justify-center", "mb-2" => $footer?->isNotEmpty()])
            >
                <div
                    class="{{ $personalize["placeholder.wrapper"] }}"
                    :class="{ 'bg-primary-100': dragging }"
                >
                    <div
                        class="{{ $personalize["placeholder.icon.wrapper"] }}"
                    >
                        <x-dynamic-component
                            :component="TallStackUi::prefix('icon')"
                    ...
                    @if (is_string($tip))
                        <p class="{{ $personalize["placeholder.tip"] }}">
                            {{ $tip }}
                        </p>
                    @else
                        {{ $tip }}
        ...
        <div
            @class([$personalize["error.wrapper"], "mb-2" => $footer?->isNotEmpty()])
            x-show="@js($error) && error"
        >
            <p
                class="{{ $personalize["error.message"] }}"
                x-text="warning"
            ></p>
        </div>
        <div
        ...
        >
            <div
                class="{{ $personalize["upload.progress"] }}"
            ...
            <div class="{{ $personalize["item.wrapper"] }}" x-ref="items">
                <ul role="list" class="{{ $personalize["item.ul"] }}">
                    @foreach ($adapter($value) as $key => $file)
                        <li
                            @class([$personalize["item.li"], "py-2" => is_array($value) && count($value) > 1])
                        >
                            <div class="flex min-w-0 gap-x-4">
                                ...
                                <div class="min-w-0 flex-auto">
                                    ...
                                    @if ($file["size"] !== null)
                                        <p
                                            class="{{ $personalize["item.size"] }}"
                                        >
                                            <span>
                                                {{ trans("tallstack-ui::messages.upload.size") }}:
                                            </span>
                                            <span>{{ $file["size"] }}</span>
                                        </p>
                                    @endif
