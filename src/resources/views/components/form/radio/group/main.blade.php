...
<fieldset {{ $attributes->only('class')->class([$customization['wrapper.base']]) }}>
    ...
    <div @class([
        ...
        @foreach ($options as $item)
            @include('ts-ui::components.form.radio.group.item', ['item' => $item, 'index' => $loop->index])
        @endforeach
    </div>
    @if ($hint && !$error)
        <x-dynamic-component :component="TallStackUi::prefix('hint')" scope="form.radio.group.hint" :$hint />
    @endif
    @if ($validate)
        <x-dynamic-component :component="TallStackUi::prefix('error')" scope="form.radio.group.error" :$property />
    @endif
</fieldset>
