...
<x-dynamic-component
    :component="TallStackUi::prefix('wrapper.input')"
    :$id
    :$property
    :$error
    :$label
...
>
    @if ($icon)
        <div
            @class([$personalize["icon.wrapper"], $personalize["icon.paddings." . $position]])
        >
            <x-dynamic-component
    ...
    @if ($clearable)
        <div
            x-data="tallstackui_formInputClearable()"
            @class([$personalize["clearable.wrapper"], $personalize["clearable.padding"], "!pr-8" => $icon && $position === "right"])
            x-show="clearable"
        >
            <button
                type="button"
                class="cursor-pointer"
                dusk="tallstackui_form_input_clearable"
            >
                <x-dynamic-component
                    :component="TallStackUi::prefix('icon')"
    ...
    >
        @if ($prefix instanceof \Illuminate\View\ComponentSlot)
            <div
                {{ $prefix->attributes->merge(["class" => $personalize["input.slot"]]) }}
            >
                {{ $prefix }}
            </div>
        @elseif (is_string($prefix))
            <span
                @class(["mr-1 ml-2", $personalize["input.slot"], $personalize["error"] => $error])
            >
                {{ $prefix }}
            </span>
        @endif
        ...
        @if ($suffix instanceof \Illuminate\View\ComponentSlot)
            <div
                {{ $suffix->attributes->merge(["class" => $personalize["input.slot"]]) }}
            >
                {{ $suffix }}
            </div>
        @elseif (is_string($suffix))
            <span
                @class(["mr-2 ml-1", $personalize["input.slot"], $personalize["error"] => $error])
            >
                {{ $suffix }}
            </span>
        @endif
