...
>
    ...
    >
        <div
            @class(["flex flex-col", "mb-2" => $helper || $footer?->isNotEmpty(), "w-full" => $format === "24"])
        >
            <div class="{{ $personalize["wrapper"] }}">
                <span
                    x-text="formatted.hours"
                    x-ref="hours"
                    class="{{ $personalize["time"] }}"
                ></span>
                <span class="{{ $personalize["separator"] }}">:</span>
                <span
                    x-text="formatted.minutes"
                    x-ref="minutes"
                    class="{{ $personalize["time"] }}"
                ></span>
                @if ($format === "12")
                    <div class="{{ $personalize["interval.wrapper"] }}">
                        <p
                            class="{{ $personalize["interval.text"] }}"
                            x-text="interval"
                        ></p>
                    </div>
                @endif
            ...
            <div wire:ignore.self class="{{ $personalize["helper.wrapper"] }}">
                <input
                    ...
                    x-on:change="alert(1)"
                    x-on:mouseenter="
                        $refs.hours.classList.add(
                            '{{ $personalize["range.light"] }}',
                            '{{ $personalize["range.dark"] }}',
                        )
                    "
                    x-on:mouseleave="
                        $refs.hours.classList.remove(
                            '{{ $personalize["range.light"] }}',
                            '{{ $personalize["range.dark"] }}',
                        )
                    "
                ...
                <input
                    ...
                    x-on:mouseenter="
                        $refs.minutes.classList.add(
                            '{{ $personalize["range.light"] }}',
                            '{{ $personalize["range.dark"] }}',
                        )
                    "
                    x-on:mouseleave="
                        $refs.minutes.classList.remove(
                            '{{ $personalize["range.light"] }}',
                            '{{ $personalize["range.dark"] }}',
                        )
                    "
            ...
            @if ($format === "12")
                <div
                    x-ref="format"
                    {{ $attributes->only("x-on:interval") }}
                    class="{{ $personalize["interval.buttons.wrapper"] }}"
                >
                    <button
                        type="button"
        ...
        @if ($helper || $footer)
            <x-slot:footer>
                @if ($helper)
                    <x-dynamic-component
                        :component="TallStackUi::prefix('button')"
