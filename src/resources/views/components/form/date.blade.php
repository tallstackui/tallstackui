...
>
        ...
        <x-slot:suffix class="mr-2 ml-1">
            <div class="{{ $personalize["icon.wrapper"] }}">
                <button
                    ...
                    x-on:click="clear()"
                    x-show="quantity > 0"
                    {{ $attributes->only(["disabled", "readonly", "x-on:clear"]) }}
                    dusk="tallstackui_date_clear"
                >
                    <x-dynamic-component
                ...
                <button
                    ...
                    class="cursor-pointer"
                    x-on:click="
                        (disables['disabled'] ?? false) || (disables['readonly'] ?? false)
                            ? false
                            : (show = ! show)
                    "
                    {{ $attributes->only(["disabled", "readonly"]) }}
                    dusk="tallstackui_date_open_close"
                >
                    <x-dynamic-component
                        :component="TallStackUi::prefix('icon')"
    ...
    >
        <div class="{{ $personalize["box.picker.button"] }}">
            <span>
                <button
                    ...
                    x-on:click="picker.month = true"
                    class="{{ $personalize["label.month"] }}"
                ></button>
                <button
                    type="button"
                    x-text="year"
                    x-on:click="
                        picker.year = true
                        range.year.start = year - 11
                    "
                    class="{{ $personalize["label.year"] }}"
                ></button>
            </span>
            <template x-if="picker.month">
                <div
                    class="{{ $personalize["box.picker.wrapper.first"] }}"
                    x-cloak
                >
                    <div
                        class="{{ $personalize["box.picker.wrapper.second"] }}"
                    >
                        <div
                            class="{{ $personalize["box.picker.wrapper.third"] }}"
                        >
                            <button
                                type="button"
                                class="{{ $personalize["box.picker.label"] }}"
                                x-on:click="
                                    if (monthYearOnly) {
                                        return false
                                    }
                            ...
                            <button
                                ...
                                class="mr-2"
                                x-on:click="now()"
                                x-show="!monthYearOnly"
                            >
                                {{ trans("tallstack-ui::messages.date.helpers.today") }}
                            </button>
                        </div>
                        <template
                            x-for="(months, index) in calendar.months"
                            :key="index"
                        >
                            <button
                                class="{{ $personalize["box.picker.range"] }}"
                                type="button"
                ...
                >
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                <span
                                    x-text="range.year.first"
                                    class="{{ $personalize["label.month"] }}"
                                ></span>
                                <span
                                    class="{{ $personalize["box.picker.separator"] }}"
                                >
                                    -
                                </span>
                                <span
                                    x-text="range.year.last"
                                    class="{{ $personalize["label.month"] }}"
                                ></span>
                            </div>
                            <button
                        ...
                        <template
                            x-for="(range, index) in yearRange()"
                            :key="index"
                        >
                            <button
                                type="button"
        ...
        <x-slot:footer>
            <div class="mb-3 grid grid-cols-7" x-show="!monthYearOnly">
                <template x-for="(day, index) in calendar.week" :key="index">
                    <div class="px-0.5">
                        <div
                            x-text="day.slice(0, 3)"
                            class="{{ $personalize["label.days"] }}"
                        ></div>
                    </div>
                </template>
            </div>
            <div class="grid grid-cols-7" x-show="!monthYearOnly">
                ...
                <template x-for="(day, index) in days" :key="index">
                    <div
                        ...
                        x-bind:class="{
                            'rounded-l-full':
                                new Date(day.instance).getTime() === new Date(date.start).getTime(),
                            'rounded-r-full w-7 h-7':
                                new Date(day.instance).getTime() === new Date(date.end).getTime(),
