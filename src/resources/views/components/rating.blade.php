...
>
    ...
    <template
        x-for="(star, index) in Array.from({ length: quantity })"
        :key="index"
    >
        <button
            type="button"
            @if ($livewire && !$static) x-on:click.prevent="evaluate('{{ $evaluateMethod }}', index + 1)" @endif
        ...
        >
            @unless ($icon)
                <svg
                    ...
                    x-bind:class="{
                        '{{ $colors["background"] }}': rate >= index + 1,
                        '{{ $personalize["star"] }}': rate < index + 1,
                    }"
                >
                    <path
                        fill-rule="evenodd"
