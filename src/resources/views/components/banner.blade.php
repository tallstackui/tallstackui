...
@if ($show)
    ...
    >
        @if ($left)
            <span
                @if (!is_string($left)) {{ $left->attributes->class([$personalize['slot.left'], $colors['text'] ?? '' => !$wire]) }} x-bind:class="{
        ...
        @if ($wire)
            <div
                @class([$personalize["text"], "flex items-center justify-center gap-2"])
            >
        ...
        @else
            <span
                @class([$personalize["text"], $colors["text"] ?? $color["text"]])
            >
                {!! $text ??= $slot->toHtml() !!}
            </span>
        @endif
