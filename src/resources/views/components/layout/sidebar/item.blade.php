...
@if ($visible)
    @if ($slot->isNotEmpty())
        <li
            x-data="{ show: @js($opened ?? (\Illuminate\Support\Str::contains($slot, "ts-ui-group-opened") ?? false)) }"
        >
            <button
    ...
    @else
        <li
            class="{{ $personalize["item.wrapper.base"] }}"
            x-bind:class="{
            ...
            <a
                ...
                @class([
                    ...
                    \Illuminate\Support\Arr::toCssClasses([
                        "ts-ui-group-opened",
                        $personalize["item.state.current"],
