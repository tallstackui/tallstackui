...
>
    <template x-for="toast in toasts" :key="toast.id">
        <div
            x-data="tallstackui_toastLoop(toast)"
            x-show="show"
            x-ref="toast"
            ...
            @class([
                ...
                "md:items-start" =>
                    $configurations["position"] === "top-left" ||
                    $configurations["position"] === "bottom-left",
                "md:items-end" =>
                    $configurations["position"] === "top-right" ||
                    $configurations["position"] === "bottom-right",
            ])
        >
            <div class="{{ $personalize["wrapper.third"] }}">
                <div class="{{ $personalize["wrapper.fourth"] }}">
                    ...
                    <div class="{{ $personalize["content.wrapper"] }}">
                        <p
                            class="{{ $personalize["content.text"] }}"
                            x-bind:class="{
                                'font-medium': ! toast.confirm,
                                'font-semibold': toast.confirm,
                            }"
                            x-html="toast.title"
                        ></p>
                        <p
                            class="{{ $personalize["content.description"] }}"
                        ...
                        ></p>
                        <template
                            x-if="toast.options && (toast.options.confirm?.text || toast.options.cancel?.text)"
                        >
                            <div
                                class="{{ $personalize["buttons.wrapper.first"] }}"
                                x-bind:class="{ 'gap-x-2': toast.options.confirm && toast.options.cancel }"
                            >
                                <button
                                    dusk="tallstackui_toast_confirmation"
                                ...
                                <div x-show="toast.options.cancel">
                                    <button
                                        dusk="tallstackui_toast_rejection"
                    ...
                    <div class="{{ $personalize["buttons.wrapper.second"] }}">
                        <div
                            class="{{ $personalize["buttons.close.wrapper"] }}"
                        >
                            <button
                                x-on:click="hide(true, false)"
                                type="button"
                        ...
                        <div
                            x-show="toast.expandable && toast.description"
                            class="{{ $personalize["buttons.expand.wrapper"] }}"
                        >
                            <button
                                dusk="tallstackui_toast_expandable"
                ...
                @if ($configurations["progress"])
                    <div
                        ...
                        class="{{ $personalize["progress.wrapper"] }}"
                    >
                        <span
                            x-ref="progress"
                            x-bind:style="`animation-duration:${toast.timeout * 1000}ms`"
                            @class(["animate-progress", $personalize["progress.bar"]])
                            x-cloak
                        ></span>
                    </div>
                @endif
