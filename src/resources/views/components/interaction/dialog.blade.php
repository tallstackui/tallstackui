...
>
    <div
        ...
        class="{{ $personalize["background"] }}"
    ></div>
    <div
        @class([$personalize["wrapper.first"], "backdrop-blur-sm" => $configurations["blur"]])
    >
        <div class="{{ $personalize["wrapper.second"] }}">
            ...
            >
                ...
                <div>
                    <div
                        class="{{ $personalize["icon.wrapper"] }}"
                        x-bind:class="{
                            '{{ $colors["icon"]["background"]["success"] }}':
                                dialog.type === 'success',
                            '{{ $colors["icon"]["background"]["error"] }}': dialog.type === 'error',
                            '{{ $colors["icon"]["background"]["info"] }}': dialog.type === 'info',
                            '{{ $colors["icon"]["background"]["warning"] }}':
                                dialog.type === 'warning',
                            '{{ $colors["icon"]["background"]["question"] }}':
                    ...
                    <div class="{{ $personalize["text.wrapper"] }}">
                        <h3
                            class="{{ $personalize["text.title"] }}"
                            x-html="dialog.title"
                        ></h3>
                        <div
                            class="{{ $personalize["text.description.wrapper"] }}"
                        >
                            <p
                                class="{{ $personalize["text.description.text"] }}"
                                x-html="dialog.description"
                            ></p>
                        </div>
                    </div>
                </div>
                <div class="{{ $personalize["buttons.wrapper"] }}">
                    ...
                    <button
                        class="{{ $personalize["buttons.confirm"] }}"
                        x-bind:class="{
                        ...
                        x-text="dialog.options?.confirm?.text ?? text.ok"
                    ></button>
                </div>
            </div>
        </div>
