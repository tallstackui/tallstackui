export default (options) => ({
  ...
  get position() {
    ...
    const anchor = this.radial ? this.pointer.x : (this.abscissa(this.active) / this.plot.width) * width
  ...
  factors() {
    ...
    const values = this.series.filter((entry, position) => this.visible(position)).flatMap((entry) => entry.data)
    ...
    return {
      factor,
      offset: this.plot.bottom - (this.plot.band * (this.scale.min - min)) / range - factor * this.plot.bottom,
    }
  },
  locate(event) {
  ...
  sector(from, to) {
    ...
    if (to - from >= 2 * Math.PI - 1e-9) {
      ...
      const arc = (distance, sweep, angle) => `A${distance},${distance} 0 1,${sweep} ${point(angle, distance)}`
      const turn = (distance, sweep) => `${arc(distance, sweep, half)} ${arc(distance, sweep, from)}`
  ...
  track(event) {
    ...
    const index = this.type === 'bar' ? Math.floor(ratio * this.length) : Math.round(ratio * (this.length - 1))
  ...
  visibleValues() {
    return this.slices.filter((slice, index) => this.visible(index)).map((slice) => Math.max(0, slice.raw))
  },
  round(value) {
