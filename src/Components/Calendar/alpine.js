...
export default (
  ...
  init() {
    ...
    this.reset();
    this.map();
    if (this.double) this.mapSecondary();
  ...
  /**
   * Sync the state and dispatch the select event.
   *
   * @return {void}
   */
  sync() {
   ...
   *
   * @param {Event} event
   * @param {Object} day  Pre-computed day object from days/daysSecondary; carries
   *                      its own `instance` (dayjs) so clicks on the secondary
   *                      panel resolve to the correct month/year.
   * @return {*}
   */
  select(event, day) {
    event.preventDefault();

    const date = day.instance;
    const formatted = day.formatted;
  ...
  /**
   * Map the days of the secondary month (month + 1).
   *
   * @return {void}
   */
  mapSecondary() {
    ...
    let secondaryMonth = this.month + 1;
    let secondaryYear = this.year;
    ...
    const start = dayjs(`${secondaryYear}-${secondaryMonth + 1}-01`);
    const month = start.endOf('month').date();
    const week = start.day();
    const count = (week - this.start + 7) % 7;

    this.blanksSecondary = Array.from({ length: count }, (key, value) => value + 1);
    ...
    this.daysSecondary = Array.from({ length: month }, (key, value) => {
      const date = start.add(value, 'day');
      const formatted = date.format('YYYY-MM-DD');
      const timestamp = date.toDate().getTime();
  ...
  refresh() {
    ...
    if (this.double) update(this.daysSecondary);
  },
  /**
  ...
  /**
   * Toggle the month picker. No-op when lockMonthYear is true.
   *
   * @return {void}
   */
  toggleMonthPicker() {
    if (this.lockMonthYear) return;

    this.picker.year = false;
    this.picker.month = !this.picker.month;
  },
  /**
   * Toggle the year picker. No-op when lockMonthYear is true.
   *
   * @return {void}
   */
  toggleYearPicker() {
    ...
    this.picker.year = !this.picker.year;
  },
  /**
  ...
  clear() {
    ...
    this.model = this.value = this.date.start = this.date.end = null;
  ...
  /**
   * Get the month index displayed in the secondary panel (0-11).
   *
   * @return {Number}
   */
  get secondaryMonth() {
    return (this.month + 1) % 12;
  },
  /**
   * Get the year displayed in the secondary panel.
   *
   * @return {Number}
   */
  get secondaryYear() {
